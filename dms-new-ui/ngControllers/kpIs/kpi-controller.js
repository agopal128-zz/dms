...
define(['angularAMD', 'kpiLetter', 'counterMeasure', 'kpiService', 'actualsController', 'counterMeasureController', 'secondaryMetricKpiController',
    'actualsService', 'configurationService', 'graphDirective', 'validNumber', 'metricType', 'notificationModalService', 'circleProgress', 'progressValueDirective'],
    function (angularAMD) {
        ...
        function ($routeParams, kpiService, actualsService, $uibModal, notificationService,
            ...
            if (scorecardName) {
                $rootScope.title = "Scorecard : " + sessionStorageService.get('scorecardName');
            }
            self.goalText = '';
        ...
        var formatDataForGraph = function (data) {
            ...
            angular.forEach(graphData, function (item, index) { 
                tempData.series[0].data.push(item.stretchGoalValue);
                tempData.series[1].data.push(item.goalValue);
                tempData.series[2].data.push(item.actualValue);
