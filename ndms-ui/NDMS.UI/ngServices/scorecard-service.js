    ...
    angularAMD.service('scorecardService', ['configService', '$http', '$log', '$q',
        function (configService, $http, $log, $q) {
            ...
            this.addOrRemoveFromExistingList = function (scorecardItemList, updatedScorecardList) {
                ...
                updatedMenuItemList.forEach(function (scorecard) {

                    scorecardItemList.pushIfNotExist(scorecard, function (elt) {
                        return scorecard.id === elt.id;
                    });
                });

                scorecardItemList.forEach(function (scorecard) {
                    if (!updatedMenuItemList.inArray(function (elt) { return scorecard.id === elt.id; })) {
                        scorecardItemList.splice(scorecardItemList.indexOf(scorecard), 1);
                    };
                });
            }

            this.updateExistingDropDownDto = function (existingDropdownDto, updatedDropdownDto) {
                existingDropdownDto.scorecardExists = updatedDropdownDto.scorecardExists;
                self.addOrRemoveFromExistingList(existingDropdownDto.kpiOwnerScorecards, updatedDropdownDto.kpiOwnerScorecards);
                self.addOrRemoveFromExistingList(existingDropdownDto.teamMemberScorecards, updatedDropdownDto.teamMemberScorecards);
                self.addOrRemoveFromExistingList(existingDropdownDto.rootScorecards, updatedDropdownDto.rootScorecards);
            }
            //get Scorecard by id
            ...
            this.getHierarchyDropdownList = function () {
                ...
                $http.get(url)
                    .success(function (data) {
                        ...
                        self.updateExistingDropDownDto(self.hierarchyDropdownDto, data.data)
            ...
            //Get hierarchy data for users with multiple scorecards
            this.getMultipleScorecardHierarchyData = function () {

                var deferred = $q.defer();
                var url = baseUrl + "GetMultipleScorecardHierarchy";
                $http.get(url)
                  .success(function (data) {
                      deferred.resolve(data.data);
                  }).error(function (msg, code) {
            ...
            this.getScorecardList = function () {

                var deferred = $q.defer();
                var url = baseUrl + "GetScorecards";
                $http.get(url)
                  .success(function (data) {
                      deferred.resolve(data.data);
                  }).error(function (msg, code) {
